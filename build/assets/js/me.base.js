...
; (function (me, win) {
    var slide = function (options) {
        ...
        this.__isStop = false;
        options = me.copy({}, this.defaults, options);
...
; (function () {
    var countdown = function (options) {
        var __self = this;
        __self.__runTime = null;
        __self.__arrTime = [];
        __self.__current = null;
        if (options instanceof Date) {
            __self.__arrTime.push(options);
        } else if (options instanceof Array) {
            for (var i in options) {
                __self.__arrTime.push(options[i]);
            }
        } else {
            if (options.arrTime) {
                for (var i in options.arrTime) {
                    __self.__arrTime.push(options[i]);
                }
            }
        }
        if (!__self.__arrTime.length) {
            me.log("倒计时参数缺省");
            return;
        }
        options = me.copy({}, this.defaults, options);
        countdown.superclass.constructor.call(this, options);
    }
    me.inherit(countdown, me.assembly);
    countdown.prototype.oninit = function () {
        var __self = this, _o = __self.options;
        __self.sortbyAsc();//对数组排序
        __self.calindex();//计算当前index
        me.log(__self.__arrTime);
        me.log(__self.__current);
    };
    countdown.prototype.sortbyAsc = function () {
        var __self = this, _o = __self.options;
        var _length = __self.__arrTime.length;
        for (var i = 0; i < _length; i++) {
            for (var j = i; j < _length; j++) {
                if (__self.__arrTime[i] > __self.__arrTime[j]) {
                    var temp = __self.__arrTime[i];
                    __self.__arrTime[i] = __self.__arrTime[j];
                    __self.__arrTime[j] = temp;
                }
            }
        }
        return __self;
    };
    countdown.prototype.calindex = function () {
        var __self = this, _o = __self.options;
        var _length = __self.__arrTime.length;
        var _nowDate = new Date();
        var curtemp = null, curindex = null, curstate = true;
        for (var i = 0; i < _length; i++) {
            var temp = __self.__arrTime[i] - _nowDate;
            if (temp > 0) {
                if (curstate) {
                    curtemp = temp;
                    curindex = i;
        ...
        __self.__current = curindex;
        return __self;
    };
    countdown.prototype.go = function () {
        var __self = this;
        var ts = (this.myDate) - (new Date());
        if (ts < 0) {
            clearTimeout(__self.__runTime);
            __self.__runTime = null;
            return;
        }
